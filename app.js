...
function renderOnboardingInfoRows(employee, locked) {
  ...
  return `<div class="subtle-card"><div class="section-header"><div><p class="eyebrow">Personal details</p><h3>Personal details</h3></div></div><p class="helper notice" style="margin-bottom: 12px; font-weight: 500;">To unlock features like Attendance, Leave, and WFH, you must fill all basic mandatory data up to Aadhar No. and click 'Save as draft'.</p><div class="stack">${personalRows}<div class="section-header profile-subsection"><div><p class="eyebrow">Address</p><h3>Present address</h3></div></div>${presentRows}<div class="section-header profile-subsection"><div><p class="eyebrow">Address</p><h3>Permanent address</h3></div><label class="sheet-select-all"><input id="sameAsPresentAddress" type="checkbox" ${sameAsPresent ? "checked" : ""} ${locked ? "disabled" : ""} />Same as present address</label></div>${permanentRows}</div></div><div class="subtle-card"><div class="section-header"><div><p class="eyebrow">Bank details</p><h3>Bank details</h3></div></div><div class="stack">${bankRows}</div></div><div class="subtle-card"><div class="section-header"><div><p class="eyebrow">Employment</p><h3>Employment details</h3></div></div><div class="stack">${employmentRows}</div></div>`;
}
function renderAttachmentUploadSection(employee, locked) {
  return `<div class="subtle-card" data-attachment-card="true"><div class="section-header"><div><p class="eyebrow">Attachments</p><h3>Upload employee documents</h3></div><span class="pill">${PROFILE_ATTACHMENT_REQUIREMENTS.filter(i => i.required !== false).length} mandatory items</span></div><div class="admin-activity-table-wrap"><table class="admin-activity-table profile-attachment-table"><thead><tr><th>Required document</th><th>Upload</th><th>Current file</th></tr></thead><tbody>${PROFILE_ATTACHMENT_REQUIREMENTS.map((item) => { const current = employee.attachments?.[item.key]; const reqStar = item.required !== false ? " *" : ""; return `<tr data-attachment-row="${item.key}"><td>${escapeHtml(item.label)}${reqStar}</td><td><input type="file" data-attachment-key="${item.key}" ${locked ? "disabled" : ""} /></td><td>${current ? `<span class="pill success">Uploaded: ${escapeHtml(current.fileName)}</span><div class="muted">${escapeHtml(current.uploadedAt || "")}</div>${current.savedFileId ? `<button type="button" class="link-btn" style="padding:4px 0; margin-right: 8px;" onclick="viewSecureAttachment('${escapeHtml(current.savedFileId)}', '${escapeHtml(current.fileName)}')">View</button><button type="button" class="link-btn" style="padding:4px 0;" onclick="downloadSecureAttachment('${escapeHtml(current.savedFileId)}', '${escapeHtml(current.fileName)}')">Download</button>` : ''}` : `<span class="muted">Not uploaded</span>`}</td></tr>`; }).join("")}</tbody></table></div></div>`;
}
...
function renderOnboardingSummary(employee) { if (!employee.hiring.onboardingSubmittedAt && !employee.hiring.profileDraftSaved) return emptyState("Employee has not submitted onboarding details yet."); return getOnboardingFieldEntries(employee).map((field) => `<div class="list-item"><strong>${escapeHtml(field.label)}${field.required ? " *" : ""}</strong><span>${escapeHtml(employee.onboardingDetails[field.key] || "-")}</span></div>`).join(""); }
...
function bindEmployeeEvents() {
  ...
  app.querySelector("#saveProfileDraftBtn")?.addEventListener("click", async () => {
    ...
    const missingKeys = basicKeys.filter(k => meta[k]?.required && !String(updatedOnboardingDetails[k] || "").trim());
    if (missingKeys.length) {
      ...
      showModalMessage("Basic fields missing", "Please fill all mandatory basic details up to Aadhar No. to save as draft.");
      return;
    }

...
function getAttendancePolicyStatus() {
  ...
  return { lat, lon, radius, configured: Number.isFinite(lat) && Number.isFinite(lon) && Number.isFinite(radius) && radius > 0 };
}
...
function renderAdminLoginForm() { return `<form id="adminLoginForm" class="stack"><div class="field"><label for="adminEmail">Admin email</label><input id="adminEmail" type="email" placeholder="admin@hrms.local" required /></div><div class="field"><label for="adminPassword">Password</label><input id="adminPassword" type="password" value="${escapeHtml(state.adminProfile.password)}" required /></div><button class="primary-btn" type="submit">Enter admin portal</button><button class="link-btn auth-helper-btn" id="adminForgotPasswordBtn" type="button">Forgot password?</button></form>`; }
...
function renderAdminAttendanceAdjustmentConsole() {
  const employeeOptions = state.employees.map((employee) => `<option value="${escapeHtml(employee.id)}" ${state.adjustmentHistoryFilterEmployee === employee.id ? "selected" : ""}>${escapeHtml(getEmployeeDisplayName(employee))} (${escapeHtml(employee.id)})</option>`).join("");
...
function renderAdminLeaveWfhConsole() {
  ...
  const employeeOptions = activeEmployees.map((employee) => `<option value="${escapeHtml(employee.id)}" ${selected?.id === employee.id ? "selected" : ""}>${escapeHtml(getEmployeeDisplayName(employee))} (${escapeHtml(employee.id)})</option>`).join("");
  const historyEmployeeOptions = activeEmployees.map((employee) => `<option value="${escapeHtml(employee.id)}" ${historyEmployeeFilter === employee.id ? "selected" : ""}>${escapeHtml(getEmployeeDisplayName(employee))} (${escapeHtml(employee.id)})</option>`).join("");
...
function renderAdminLeaveWfhReportEmployeeSuggestionButtons(query) {
  return getAdminLeaveWfhReportEmployeeSuggestions(query)
    .map((employee) => `<button class="report-employee-suggestion" type="button" data-admin-leave-wfh-report-employee="${escapeHtml(`${getEmployeeDisplayName(employee)} ${employee.id}`)}"><strong>${escapeHtml(getEmployeeDisplayName(employee))}</strong><span>${escapeHtml(employee.id)}</span></button>`)
    .join("");
}
...
function createEmployeeFromForm(sendOffer) {
  ...
  const signupCode = app.querySelector("#newEmployeeCode")?.value.trim();
  const password = app.querySelector("#newEmployeePassword")?.value.trim() || TEMP_PASSWORD;
  if (!fullName || !email || !department || !role || !signupCode) {
    showModalMessage("Employee details are incomplete", "Please complete all employee creation fields before continuing.");
    return;
  }
  if (state.employees.some((item) => item.email.toLowerCase() === email)) {
    showModalMessage("Employee already exists", "An employee with this email address is already registered in the portal.");
    return;
  }
  const employee = {
    id: nextEmployeeId(),
